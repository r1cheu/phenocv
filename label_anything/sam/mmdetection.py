...
def load_my_model(model_name='sam_hq',
                  device='cuda:0',
                  sam_config='vit_b',
    ...
    if model_name == 'sam':
        try:
            ...
            raise ModuleNotFoundError(
                ...
                'run `pip install segment_anything` to install')
...
class MMDetection(LabelStudioMLBase):
    ...
    def __init__(self,
        ...
        PREDICTOR = load_my_model(model_name, device, sam_config,
                                  sam_checkpoint_file)
        ...
        # Collect label maps from `predicted_values="airplane,car"` attribute in <Label> tag # noqa
        ...
        # self.model = init_detector(config_file, checkpoint_file, device=device) # noqa
    ...
    def predict(self, tasks, **kwargs):
        ...
        # converting the mask from the model to RLE
        # format which is usable in Label Studio
        ...
        if self.out_poly:
            ...
            ).remove_near_neighbors(
                points_list
            )  # remove near neighbors (increase distance_threshold to reduce more points) # noqa
