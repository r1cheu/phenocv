...
class SamOnnxModel(nn.Module):
    ...
    It combines the prompt encoder, mask decoder, and mask postprocessing of
    Sam, with some functions modified to enable model tracing. Also supports
    extra options controlling what information. See the ONNX export script for
    details.
    """
    ...
    def _embed_points(self, point_coords: torch.Tensor,
        ...
        point_embedding = point_embedding + self.model.prompt_encoder.not_a_point_embed.weight * (  # noqa
            point_labels == -1)

        for i in range(self.model.prompt_encoder.num_point_embeddings):
            point_embedding = point_embedding + self.model.prompt_encoder.point_embeddings[  # noqa
                i].weight * (
                    point_labels == i)
    ...
    def _embed_masks(self, input_mask: torch.Tensor,
        ...
        mask_embedding = has_mask_input * self.model.prompt_encoder.mask_downscaling(  # noqa
            input_mask)
